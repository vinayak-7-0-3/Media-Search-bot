...
async def filter(client, message):
    ...
    if 2 < len(message.text) < 100:    
        ...
        if files:
            ...
            await client.send_message(
                chat_id=message.from_user.id,
                text="Sorry, I couldn't find the apk. Check if Spelling is correct."
            )
            await client.send_sticker(chat_id=message.from_user.id, sticker='CAADBAADMwADX8YBGU_ftR4iOcFcAg')
            return
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if (clicked == typed):
        ...
        if query.data.startswith("next"):
            ...
            await query.message.edit(text="Bot By MIUI Icons for searching icon pack apks.", reply_markup=InlineKeyboardMarkup(buttons), disable_web_page_preview=True)
        ...
        await query.answer("Why are you trying to click someone else's search 🤨",show_alert=True)
